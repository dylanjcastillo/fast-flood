...
function game_finished(has_won = false, color_number = 0) {
    ...
    if (has_won) {
        ...
        var msg_time = String(time) + " seconds";
        var msg_moves = String(moves) + " moves";
        if (tries < 2) {
    ...
    if (!has_won) {
        ...
        if (tries < max_tries) {
            $('#message-part-2').html("I know you can do better.<br><br>Try again?");
        } else {
            $('#message-part-2').text("Hope to see you again tomorrow!");
        }
    }
...
$("#start-btn").on("click", function () {
    ...
    document.getElementById("buttons").scrollIntoView(false);
